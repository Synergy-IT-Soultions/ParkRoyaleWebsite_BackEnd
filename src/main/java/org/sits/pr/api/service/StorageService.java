...
public class StorageService {
	...
	public ImageFileInfo uploadImage(MultipartFile file, String imageType)
			throws IllegalStateException, IIOException, IOException {
	...
	public void deleteContainerImage(ContainerImageInfo containerImageInfo, Long updatedBy) {
		containerImageInfo.setContainerImageIsActive(0);
		containerImageInfo.setUpdatedBy(updatedBy);
		containerImageInfoRepository.save(containerImageInfo);
	}
	
	public void deleteImage(Long imageInfoId, Long updatedBy) {
		ImageInfo imageInfo = imageInfoRepository.findByImageInfoIdAndImageIsActive(imageInfoId, Integer.valueOf(1));

		List<ContainerImageInfo> containerImageInfoList = containerImageInfoRepository.findByImageInfoId(imageInfoId);
		if (containerImageInfoList != null && !containerImageInfoList.isEmpty())
			containerImageInfoList.forEach(containerImageInfo -> deleteContainerImage(containerImageInfo, updatedBy));
